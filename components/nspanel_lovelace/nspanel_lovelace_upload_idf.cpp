...
// Adapted from: https://github.com/esphome/esphome/blob/dc0a7b1e205f5fa4e25fd1cadd507c27173636e1/esphome/components/nextion/nextion_upload_idf.cpp
...
int NSPanelLovelace::upload_range_(const std::string &url, int range_start) {
...
bool NSPanelLovelace::upload_tft(const std::string &url) {
  ...
  ESP_LOGVV(TAG, "Available heap: %" PRIu32, esp_get_free_heap_size());
  // TODO: review this
  // this->send_nextion_command_("sleep=0"); // is this a valid command?
  ...
  ESP_LOGD(TAG, "Updating tft from \"%s\" with a file size of %d, Heap Size %" PRIu32, url.c_str(),
